...
def show_decision_boundary(X, Y, P,
    ...
    dbplot.plot(
        show_indices=False,
        graph_separators=P,
        scatter_size_scale=.1, legend=True, generate_testpoints=False).show()
...
def test_in_one(n_dim, batch_size, n_iter, C, alpha, compute_emd=True, k_nbrs=3, emd_method=emd_samples):
    ...
    train_rep(model_ae, 0.01, X, P, n_iter, 10, batch_size, alpha=0,
              C_reg=0, compute_emd=compute_emd, adv=False, verbose=True)

    # AE_P.
    model_ae_P = FairRep(len(X[0]) - 1, n_dim - 1)
    train_rep(model_ae_P, 0.01, X_no_p, P, n_iter, 10, batch_size,
              alpha=0, C_reg=0, compute_emd=compute_emd, adv=False, verbose=True)
    ...
    X = torch.tensor(X).float()
    P = torch.tensor(P).long()
    train_rep(model_nfr, 0.01, X, P, n_iter, 10, batch_size, alpha=alpha,
              C_reg=0, compute_emd=compute_emd, adv=True, verbose=True)
    results = {}
    ...
    performance.append(get_consistency(
        X.data.cpu().numpy(), lin_model, n_neighbors=k_nbrs))
    ...
    performance.append(get_consistency(
        X[:, :-1].data.cpu().numpy(), lin_model, n_neighbors=k_nbrs))
    print('calculating stat diff...')
    performance.append(stat_diff(X[:, :-1].data.cpu().numpy(), P, lin_model))
    performance.append(equal_odds(
        X[:, :-1].data.cpu().numpy(), y, P, lin_model))
    ...
    def calc_perf(y_test, y_test_scores, P_test, U, U_0, U_1, U_np, lin_model, X_test, model_name):
        print('logistic regression evaluation...')
        performance = list(evaluate_performance_sim(
            y_test, y_test_scores, P_test))
        print('calculating emd...')
        performance.append(emd_method(U_0, U_1))
        print('calculating consistency...')
        performance.append(get_consistency(
            U_np, lin_model, n_neighbors=k_nbrs, based_on=X_ori_np))
        print('calculating stat diff...')
    ...
    performance = calc_perf(y_test, y_test_scores, P_test,
                            U, U_0, U_1, U_np, lin_model, X_test, model_name)
    results[model_name] = (performance)
...
filepath = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), 'data', 'compas_clean.csv')
...
print(cal_emd_resamp(X[:, :-1][(y == 1) & (P == 0)],
                     X[:, :-1][(y == 1) & (P == 1)], 50, 10))
print('original negative group distance without P:')
print(cal_emd_resamp(X[:, :-1][(y == 0) & (P == 0)],
                     X[:, :-1][(y == 0) & (P == 1)], 50, 10))
...
print('{0:40}: {1}'.format('method', ' '.join(
    ['ks', 'recall', 'precision', 'f1', 'stat', 'emd', 'cons', 'stat_abs', 'eq_odds'])))
for key, val in results.items():
    print('{0:40}: {1}'.format(key, ' '.join(
        [str(np.round(x, 3)) for x in val]).ljust(35)))
