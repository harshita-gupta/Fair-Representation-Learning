...
class DBPlot(BaseEstimator):
    ...
    def fit(self, X, y, training_indices=None):
        ...
        if training_indices is None:
            ...
            train_idx, test_idx = train_test_split(np.arange(len(y)), test_size=training_indices)
        else:
        ...
        self.X2d_xmin, self.X2d_xmax = np.min(
            self.X2d[:, 0]), np.max(self.X2d[:, 0])
        self.X2d_ymin, self.X2d_ymax = np.min(
            self.X2d[:, 1]), np.max(self.X2d[:, 1])
        ...
        from_idx = list(random.sample(
            list(np.arange(len(self.Xminor))), n_samples))
        to_idx = list(random.sample(
            list(np.arange(len(self.Xmajor))), n_samples))
        ...
        while i < self.n_interpolated_keypoints:
            ...
            if self.decision_boundary_distance(
                ...
                # hypersphere radius half the distance between
                #  from and to db keypoints
                R = euclidean(from_point, to_point) / 2.0
                # search around either source or target keypoint,
                # with 0.5 probability,
                # hoping to find decision boundary in between
                if random.random() > 0.5:
                    from_point = to_point

                # optimize to find new db keypoint on hypersphere
                # surphase around known keypoint
                ...
                if (
                    ...
                    if retries > self.hypersphere_max_retry_budget:
                        ...
                        print(msg.format(
                            dist, self.hypersphere_max_retry_budget))
                    elif self.verbose:
                        dist = self.decision_boundary_distance(db_point)
                        print(
    ...
    def plot(
        ...
        legend=True,
        show_indices=True,
        graph_separators=None,
        model_save_name=None
    ):
        """Plots the dataset and the identified decision boundary in 2D.
        (If you wish to create custom plots,
        get the data using generate_plot() and plot it manually)
        ...
        generate_testpoints : boolean, optional (default=True)
            Whether to generate demo points around the estimated decision
            boundary as a sanity check

        generate_background : boolean, optional (default=True)
            Whether to generate faint background plot (using
            prediction probabilities of a fitted suppor vector machine,
            trained on generated demo points) to aid visualization

        tune_background_model : boolean, optional (default=False)
            Whether to tune the parameters of the support vector machine
        ...
        show_indices: boolean, optional (default=True)
            Whether to label data points with their indices

        graph_separators: array-like, shape = [n_samples],
        , optional (default=None). Array should indicate which "class"
        each sample belongs to, and will generate a separate graph
        for each class. Can be used to detect bias in machine learning by
        seeing how decision boundary affects different classes of observations
        differently.

        Returns
        -------
        plt : The matplotlib.pyplot or axis object which has
        been passed in, after plotting the data and decision
        boundary on it. (plt.show() is NOT called
        ...
        if generate_background and generate_testpoints:
            try:
                plt.imshow(
                    ...
                    extent=[self.X2d_xmin, self.X2d_xmax,
                            self.X2d_ymin, self.X2d_ymax],
                    cmap="GnBu",
                    alpha=0.33,
                )
            except (Exception, ex):
        ...
        all_plots = []
        print("unique values, ", np.unique(graph_separators))
        for clas in np.unique(graph_separators):
        
            # fltr = np.equal(graph_separators, clas)
            # masking indices:
            train_mask = graph_separators[self.train_idx] == clas
            train_idx_masked = self.train_idx[train_mask]
            test_mask = graph_separators[self.test_idx] == clas
            test_idx_masked = self.test_idx[test_mask]
            ...
            if generate_testpoints:
                plt.scatter(
                    ...
                    alpha=0.6,
                    label="Generated demo data"
                )

            # training data
            plt.scatter(
                self.X2d[train_idx_masked, 0],
                self.X2d[train_idx_masked, 1],
                40 * scatter_size_scale,
                facecolor=["g" if i else "b" for i in self.y[train_idx_masked]],
                edgecolor=[
                    "g"
                    if ((self.y_pred)[(train_idx_masked)[i]] == (self.y)[(train_idx_masked)[i]]) == 1
                    else (
                        "b"
                        if ((self.y_pred)[(train_idx_masked)[i]] == (self.y)[(train_idx_masked)[i]]) == 0
                        else "r"
                    )
                    for i in range(len(train_idx_masked))
                ],
                linewidths=5 * scatter_size_scale,
                label="Actual data (training set)"
            )
            # print(training_data_scatter.legend_elements())

            # testing data
            plt.scatter(
                self.X2d[test_idx_masked, 0],
                self.X2d[test_idx_masked, 1],
                150 * scatter_size_scale,
                facecolor=["g" if i else "b" for i in self.y[test_idx_masked]],
                edgecolor=[
                    "g"
                    if self.y_pred[test_idx_masked[i]] == self.y[test_idx_masked[i]] == 1
                    else (
                        "b"
                        if self.y_pred[test_idx_masked[i]] == self.y[test_idx_masked[i]] == 0
                        else "r"
                    )
                    for i in range(len(test_idx_masked))
                ],
                linewidths=5 * scatter_size_scale,
                marker="s",
                label="Testing data"
            ...
            if len(test_idx_masked) == 0:
                print("No demo performance calculated, as no testing data was specified")
            else:
                freq = np.array(
                    np.unique(self.y[test_idx_masked], return_counts=True)
                ).T.astype(float)
                imbalance = np.round(
                    np.max((freq[0, 1], freq[1, 1])) / len(test_idx_masked), 3
                )
                acc_score = np.round(
                    accuracy_score(self.y[test_idx_masked],
                                   self.y_pred[test_idx_masked]), 3
                )
                f1 = np.round(
                    f1_score(self.y[test_idx_masked], self.y_pred[test_idx_masked]), 3
                )
                plt.title(
                    "Test accuracy: "
                    + str(acc_score)
            ...
            if model_save_name:
                plt.savefig(model_save_name + str(clas) + ".png", bbox_inches="tight")
            plt.show()
        return
        ...
        if len(self.X_testpoints) == 0 and generate_testpoints:
            ...
            if generate_background and generate_testpoints:
                if tune_background_model:
                    ...
                    grid.fit(
                        np.vstack(
                            (self.X2d[self.train_idx], self.X_testpoints_2d)),
                ...
                self.background_model = SVC(
                    ...
                    np.vstack((self.X2d[self.train_idx],
                               self.X_testpoints_2d)),
                    np.hstack((self.y[self.train_idx], self.y_testpoints)),
                )
                xx, yy = np.meshgrid(
                    np.linspace(self.X2d_xmin, self.X2d_xmax,
                                background_resolution),
                    np.linspace(self.X2d_ymin, self.X2d_ymax,
                                background_resolution),
                )
        ...
        for i in range(len(self.decision_boundary_points)):
            ...
            for j in range(self.n_generated_testpoints_per_keypoint - 2):
                ...
                for try_i in range(tries):
                    ...
                    if (
                        euclidean(
                            testpoint2d, self.decision_boundary_points_2d[i])
                        <= max_radius
                    ):
    ...
    def _find_decision_boundary_along_line(
        ...
        def objective(l, grad=0):
            ...
            if penalize_tangent_distance:
                ...
                x1, y1 = self.dimensionality_reduction.transform([from_point])[
                    0]
